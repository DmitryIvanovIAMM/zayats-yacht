import * as ids from './ids';

export const REGULAR_SHIP_STOPS_2020 = [
  {
    _id: '41224d776a326fb40f022210',
    sailingId: ids.grandPrixSailingId,
    portId: ids.fortLauderdalePortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-04-01'),
    departureOn: new Date('2020-04-05'),
    miles: 0,
    daysAtSea: 0,
    daysInPort: 4
  },
  {
    _id: '41224d776a326fb40f022211',
    sailingId: ids.grandPrixSailingId,
    portId: ids.palmaDeMallorcaPortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-04-19'),
    departureOn: new Date('2020-04-21'),
    miles: 4262,
    daysAtSea: 14,
    daysInPort: 2
  },
  {
    _id: '41224d776a326fb40f022212',
    sailingId: ids.grandPrixSailingId,
    portId: ids.genoaPortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-04-22'),
    departureOn: new Date('2020-04-25'),
    miles: 441,
    daysAtSea: 1,
    daysInPort: 3
  },
  {
    _id: '41224d776a326fb40f022213',
    sailingId: ids.summerMediterraneanSailingId,
    portId: ids.fortLauderdalePortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-05-11'),
    departureOn: new Date('2020-05-16'),
    miles: 4659,
    daysAtSea: 16,
    daysInPort: 5
  },
  {
    _id: '41224d776a326fb40f022214',
    sailingId: ids.summerMediterraneanSailingId,
    portId: ids.palmaDeMallorcaPortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-05-31'),
    departureOn: new Date('2020-06-02'),
    miles: 4262,
    daysAtSea: 14,
    daysInPort: 2
  },
  {
    _id: '41224d776a326fb40f022215',
    sailingId: ids.summerMediterraneanSailingId,
    portId: ids.genoaPortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-06-03'),
    departureOn: new Date('2020-06-07'),
    miles: 441,
    daysAtSea: 1,
    daysInPort: 4
  },
  {
    _id: '41224d776a326fb40f022216',
    sailingId: ids.summerMediterraneanSailingId,
    portId: ids.fethiyePortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-06-11'),
    departureOn: new Date('2020-06-14'),
    miles: 1169,
    daysAtSea: 4,
    daysInPort: 3
  },
  {
    _id: '41224d776a326fb40f022217',
    sailingId: ids.europeToAsiaSailingId,
    portId: ids.hongKongPortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-07-07'),
    departureOn: new Date('2020-07-09'),
    miles: 6843,
    daysAtSea: 23,
    daysInPort: 2
  },
  {
    _id: '41224d776a326fb40f022218',
    sailingId: ids.asiaToNorthAmericaSummerSailingId,
    portId: ids.victoriaPortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-07-28'),
    departureOn: new Date('2020-07-30'),
    miles: 5680,
    daysAtSea: 19,
    daysInPort: 2
  },
  {
    _id: '41224d776a326fb40f022219',
    sailingId: ids.northAmericaEastboundSummerSailingId,
    portId: ids.ensenadaPortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-08-03'),
    departureOn: new Date('2020-08-05'),
    miles: 1201,
    daysAtSea: 4,
    daysInPort: 2
  },
  {
    _id: '41224d776a326fb40f022220',
    sailingId: ids.northAmericaEastboundSummerSailingId,
    portId: ids.golfitoPortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-08-13'),
    departureOn: new Date('2020-08-15'),
    miles: 2494,
    daysAtSea: 8,
    daysInPort: 2
  },
  {
    _id: '41224d776a326fb40f022221',
    sailingId: ids.northAmericaEastboundSummerSailingId,
    portId: ids.fortLauderdalePortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-08-20'),
    departureOn: new Date('2020-08-21'),
    miles: 1605,
    daysAtSea: 5,
    daysInPort: 1
  },
  {
    _id: '41224d776a326fb40f022222',
    sailingId: ids.fortLauderdaleBoatShowSailingId,
    portId: ids.genoaPortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-09-22'),
    departureOn: new Date('2020-09-24'),
    miles: 4659,
    daysAtSea: 32,
    daysInPort: 2
  },
  {
    _id: '41224d776a326fb40f022223',
    sailingId: ids.fortLauderdaleBoatShowSailingId,
    portId: ids.palmaDeMallorcaPortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-09-26'),
    departureOn: new Date('2020-09-30'),
    miles: 441,
    daysAtSea: 1,
    daysInPort: 4
  },
  {
    _id: '41224d776a326fb40f022224',
    sailingId: ids.fortLauderdaleBoatShowSailingId,
    portId: ids.fortLauderdalePortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-10-14'),
    departureOn: new Date('2020-10-16'),
    miles: 4262,
    daysAtSea: 14,
    daysInPort: 2
  },
  {
    _id: '41224d776a326fb40f022225',
    sailingId: ids.christmasSailingId,
    portId: ids.genoaPortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-10-31'),
    departureOn: new Date('2020-11-03'),
    miles: 4659,
    daysAtSea: 16,
    daysInPort: 3
  },
  {
    _id: '41224d776a326fb40f022226',
    sailingId: ids.christmasSailingId,
    portId: ids.palmaDeMallorcaPortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-11-05'),
    departureOn: new Date('2020-11-07'),
    miles: 441,
    daysAtSea: 1,
    daysInPort: 2
  },
  {
    _id: '41224d776a326fb40f022227',
    sailingId: ids.christmasSailingId,
    portId: ids.tortolaPortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-11-19'),
    departureOn: new Date('2020-11-22'),
    miles: 3757,
    daysAtSea: 13,
    daysInPort: 3
  },
  {
    _id: '41224d776a326fb40f022228',
    sailingId: ids.westboundNorthAmericaAndAsiaChristmasSailingId,
    portId: ids.fortLauderdalePortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-11-26'),
    departureOn: new Date('2020-12-01'),
    miles: 974,
    daysAtSea: 3,
    daysInPort: 5
  },
  {
    _id: '41224d776a326fb40f022229',
    sailingId: ids.westboundNorthAmericaAndAsiaChristmasSailingId,
    portId: ids.golfitoPortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-12-06'),
    departureOn: new Date('2020-12-08'),
    miles: 1600,
    daysAtSea: 5,
    daysInPort: 2
  },
  {
    _id: '41224d776a326fb40f022230',
    sailingId: ids.westboundNorthAmericaAndAsiaChristmasSailingId,
    portId: ids.ensenadaPortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-12-16'),
    departureOn: new Date('2020-12-18'),
    miles: 2500,
    daysAtSea: 8,
    daysInPort: 2
  },
  {
    _id: '41224d776a326fb40f022231',
    sailingId: ids.westboundNorthAmericaAndAsiaChristmasSailingId,
    portId: ids.victoriaPortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2020-12-20'),
    departureOn: new Date('2020-12-22'),
    miles: 1200,
    daysAtSea: 4,
    daysInPort: 2
  },
  {
    _id: '41224d776a326fb40f022232',
    sailingId: ids.westboundNorthAmericaAndAsiaChristmasSailingId,
    portId: ids.hongKongPortId,
    shipId: ids.industrialGuideShipId,
    arrivalOn: new Date('2021-01-21'),
    departureOn: new Date('2021-01-21'),
    miles: 5680,
    daysAtSea: 19,
    daysInPort: 2
  }
];
